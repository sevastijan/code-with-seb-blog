...
**When to use:** Slack notifications, external logging, team dashboards, CI/CD triggers. For the full GitHub Actions and scheduled-task setup, see my [headless mode CI/CD playbook](/blog/claude-code-headless-mode-cicd-automation-playbook) — hooks plus `claude -p` is the automation stack that actually holds up in production.
...
| Complex multi-agent coordination | **[Agent teams](/blog/claude-code-remote-control-dispatch-guide)** — dedicated orchestration |
...
The highest-value hook I run isn't in the list below: a `PostToolUse` matcher on `Write|Edit` that runs the tests related to the file that just changed. It turns discipline you'd otherwise have to remember into something the harness enforces — see [test-driven agentic development](/blog/test-driven-agentic-development-guide) for why immediate red-test feedback beats discovering failures at the end of a long autonomous run.
