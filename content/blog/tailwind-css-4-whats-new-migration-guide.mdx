...
featured: false
author: 'Sebastian'
excerpt: 'Tailwind CSS 4 brings a new Oxide engine, CSS-first configuration, and 100x faster builds. But is it worth migrating your existing project? Here''s what changed, what might break, and how to decide.'
---
...
This means **no IE11, no older Safari on iOS 15**, and no older Android WebViews. The framework uses modern CSS features (`@property`, `color-mix()`, cascade layers) that simply don't work in older browsers — the same wave of [now-baseline CSS that's replacing entire categories of JavaScript](/blog/modern-css-2026-has-scope-container-queries-guide).
...
Third-party plugins may not be v4-compatible yet — the same failure pattern that makes [the Vite 8 and Rolldown migration](/blog/vite-8-rolldown-migration-what-breaks) easy or painful depending entirely on your plugin chain. Check each one:
