...
featured: false
author: 'Sebastian'
excerpt: 'TanStack has grown from a data-fetching library into a complete frontend ecosystem. But with Query, Router, Table, Form, Store, DB, AI, and Start - how do you know which tools you actually need? Here''s my practical guide to building with TanStack in 2026.'
---
...
Start is TanStack's answer to Next.js and Remix. It's built on Vite and TanStack Router, with SSR, server functions, and streaming built in. Worth noting that "Remix" now means two different things — the framework people compared Start against became React Router v7, while [Remix 3 is a separate framework with no React at all](/blog/remix-3-no-react-honest-assessment) and no migration path from Remix 2.
