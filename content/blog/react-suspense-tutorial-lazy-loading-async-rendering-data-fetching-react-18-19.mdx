...
featured: false
author: 'Sebastian'
excerpt: 'Master React Suspense — from lazy loading components and data fetching with the use() hook to seamless integration with React Router and Next.js. A practical, TypeScript-based tutorial with real-world examples and common pitfalls.'
---
...
In this example, `HeavyComponent` is not part of the initial bundle. It will load only when React renders it inside `<Suspense>`. This reduces initial load size and improves performance — and when paired with [React Compiler's automatic memoization](/blog/react-compiler-deep-dive-automatic-memoization-guide), the re-render performance of these lazy-loaded components is optimized automatically. Remember to always include a `fallback`; without it, the lazy-loaded component would throw an error. Lazy loading can be applied to any component (not just routes), but it’s most common for pages or rarely used parts of the app.
