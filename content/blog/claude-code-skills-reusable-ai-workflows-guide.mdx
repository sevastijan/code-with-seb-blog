...
featured: false
author: 'Sebastian'
excerpt: 'Skills, slash commands, subagents, MCP - Claude Code has four ways to extend capabilities. Most developers use the wrong one. Here''s the decision framework that cuts research time from weeks to days.'
---
...
**Use [Hooks](/blog/claude-code-hooks-custom-agents-ai-pipelines-guide) when:**

- You need to intercept tool calls or lifecycle events
- Enforcing security gates or auto-formatting
- Building self-validating CI/CD-style pipelines
...
With 10 skills installed, you're paying ~1,000 tokens total—not the 50,000+ you'd pay if everything loaded upfront. Combined with the other [token optimization techniques](/blog/claude-code-token-optimization-bill-cut-guide) — `opusplan`, a trimmed CLAUDE.md, `.claudeignore` — skills are a major lever on long-session cost.
...
A note before the template: on newer models — especially Fable 5 — skills written as rigid step recipes constrain the model rather than help it. State goals and constraints, not procedures; my [de-prescribing guide](/blog/fable-5-prompting-de-prescribing-guide) covers the rewrite patterns.
...
Once a skill is worth sharing beyond a single repo, the next step is packaging it as a plugin and putting it in a marketplace — that's how you get versioning and automatic updates instead of copy-paste. I walk through the whole path, including the versioning trap that silently stops your users receiving fixes, in [Ship Your Own Claude Code Plugin](/blog/claude-code-plugin-marketplace-publish-guide).
