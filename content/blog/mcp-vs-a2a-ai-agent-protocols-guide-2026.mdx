...
featured: false
author: 'Sebastian'
excerpt: 'Confused by MCP, ACP, and A2A? Here''s what most guides won''t tell you: ACP merged into A2A, and MCP has documented security breaches. This guide gives you a simple decision framework: MCP for tools, A2A for agents.'
---
...
Here's the current state: **MCP for tools, A2A for agents**. That's it. And if you're implementing either, there are security implications that few articles mention — including the [MCP STDIO RCE flaw Anthropic declined to patch](/blog/mcp-rce-vulnerability-stdio-flaw-guide) that exposed roughly 200,000 servers in April 2026.
...
When you connect Claude Desktop to a database via MCP, you're running an MCP server that exposes database queries as tools. Claude (the client) discovers these tools and can invoke them during conversations. The same discovery model is now coming to the browser itself — [WebMCP lets a web page expose its own tools](/blog/webmcp-website-ai-agent-tools-guide) to in-browser agents, no server process required.
...
**Most projects only need MCP.** A2A becomes necessary when you're building [multi-agent systems](/blog/claude-code-sub-agents-multi-agent-systems-guide) where agents collaborate as peers rather than one agent using another as a tool.
...
1. **Tool Poisoning**: Malicious instructions hidden in tool descriptions that manipulate LLM behavior — I cover the full taxonomy of these [prompt injection attacks](/blog/prompt-injection-attacks-llm-security-guide) with defense strategies
...
Since this guide was written, MCP itself changed substantially: the [2026-07-28 revision went stateless](/blog/mcp-2026-07-28-stateless-spec-migration-guide), dropping sessions and the `initialize` handshake and deprecating roots, sampling and logging. The MCP-for-tools / A2A-for-agents split below still holds — but if you maintain a server, read that migration first.
