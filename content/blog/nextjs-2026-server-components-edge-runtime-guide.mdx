...
2. **Server Actions replace most API routes.** API routes only exist for external webhooks and third-party integrations. Remember that every action compiles to a public endpoint — I walk through the hardening layers in [Next.js Server Actions Security](/blog/nextjs-server-actions-security). And before you make actions your entire backend, know [when the folder-backend model bites](/blog/nextjs-server-first-architecture-backend-folder).
