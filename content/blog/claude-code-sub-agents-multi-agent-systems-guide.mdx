...
featured: false
author: 'Sebastian'
excerpt: 'Anthropic''s internal tests show multi-agent systems outperform single agents by 90%. But most tutorials stop at YAML configs. Here''s how to build production-ready orchestrator patterns with cost optimization and debugging techniques that actually work.'
---
...
The key insight for most of 2026: subagents could not spawn their own subagents, which prevented infinite nesting and kept the architecture predictable. That constraint has since been lifted — [Claude Code now supports nested subagents up to five levels deep](/blog/claude-code-nested-subagents-agent-hierarchies-guide) — but the flat, single-level model described here is still the right default for most work, and everything below assumes it.
...
When you enter plan mode (`/plan`), Claude delegates research to the Plan subagent. It explores your codebase, understands patterns, and returns findings so Claude can propose a coherent strategy. The [full Plan Mode production workflow](/blog/claude-code-plan-mode-ultraplan-production-guide) covers the annotation loop and `opusplan` cost trick in depth.
...
1. **You need domain-specific expertise** (security auditing against [prompt injection attacks](/blog/prompt-injection-attacks-llm-security-guide), performance profiling)
2. **You want opinionated behavior** (enforce coding standards, reject bad patterns — you can even [bake hooks directly into agent definitions](/blog/claude-code-hooks-custom-agents-ai-pipelines-guide) for hard guardrails)
...
The lead agent doesn't do the grunt work. It thinks strategically and delegates tactically. If hand-wiring this orchestration sounds like work, it is — and as of Opus 4.8 you can let Claude generate it for you with [dynamic workflows that spawn hundreds of parallel subagents](/blog/claude-code-dynamic-workflows-parallel-subagents-guide) from a single prompt.
...
**Critical insight from Anthropic's research:** Token usage explains 80% of performance variance. More tokens = better results. Multi-agent systems use ~15x more tokens than chat interactions, but that investment pays off in quality — provided you apply the [token optimization practices](/blog/claude-code-token-optimization-bill-cut-guide) that prevent the 15x from becoming 50x.
...
This is where most developers leave money on the table. Strategic model selection can cut costs by 70% without sacrificing quality. (With the Claude 5 family this matters even more — I wrote a dedicated [Fable 5 model-routing playbook](/blog/fable-5-claude-code-model-routing-playbook) for the four-tier lineup.)
...
**Solution:** Put critical constraints in your [`CLAUDE.md` file](/blog/claude-md-memory-persistent-context-guide), which persists across compaction.
...
- [Claude Agent SDK: Build Production AI Agents in TypeScript](/blog/claude-agent-sdk-typescript-production-guide) - Complete guide to building custom agents with the SDK
- [Claude Code Remote Control & Dispatch](/blog/claude-code-remote-control-dispatch-guide) - Programmatic agent control for headless pipelines
