...
featured: false
author: 'Sebastian'
excerpt: 'Learn how to implement Google login in a Vite + React application using @react-oauth/google, handle JWT tokens securely on the frontend, and prepare your app for user session management, personalization, and future backend integration'
---
...
4. Go to **Credentials** > **Create Credentials** > **OAuth 2.0 Client ID**:
   ...
   - Add `http://localhost:5173` to **Authorized JavaScript Origins** (and if you plan to host your app, make sure to also include your production URL, e.g., `https://www.codewithseb.com`)
...
Any user-generated input — whether it comes from form fields, URL query parameters, or external APIs — should always be treated as untrusted and potentially malicious. Never assume the frontend or backend will “just work” with whatever comes in. For example, a query string like `?name=<script>alert("XSS")</script>` can easily be reflected into the UI if not properly escaped or validated. Even seemingly innocent inputs like usernames or messages can be crafted to inject markup, styles, or scripts if you're rendering them dynamically.
