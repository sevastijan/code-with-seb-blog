...
Here's the thing — I'm not anti-AI coding. I use these tools daily. But there's a gap between "the code runs" and "the code is secure," and AI tools are spectacularly bad at closing it — especially when it comes to threats like [prompt injection, the #1 LLM security vulnerability](/blog/prompt-injection-llm-security-guide). So let me walk you through exactly what I find every time I audit a vibe-coded codebase, and the checklist I use to catch 90% of issues before they ship.
...
When you prompt an AI to "build a user authentication system," it builds something that authenticates users. It doesn't add rate limiting. It doesn't hash passwords with bcrypt — it might use MD5 or even store them in plain text if the training data skews that way. It doesn't implement account lockout after failed attempts. It doesn't add CSRF tokens. It doesn't set security headers. If the app is built on Next.js server actions, those gaps map exactly to the layers I describe in [Next.js Server Actions Security](/blog/nextjs-server-actions-security).
...
Auditing for known CVEs is only half the job, though. A freshly published package has no CVE yet — which is exactly the window supply chain worms live in. Pair `npm audit` with [install-script blocking and a release-age cooldown](/blog/npm-install-scripts-hardening-keyv-playbook) so a poisoned version can't execute before anyone has had time to notice it.

## Tools That Catch These Automatically

Manual audits are thorough but slow. Here's the toolchain I use to automate the first pass — and note that AI-driven scanning is now a layer of its own, with [Claude Security's multi-agent scan](/blog/claude-security-plugin-multi-agent-scanner-guide) sitting between in-session guidance and CI-time SAST.
...
- **SonarQube** — Great for CI/CD integration. Set it as a quality gate and block merges that introduce vulnerabilities. For AI-generated diffs specifically, [Claude Code's /ultrareview cloud agents](/blog/claude-code-ultrareview-cloud-bug-hunting-guide) catch concurrency and boundary bugs that pattern-based SAST misses.
- **CodeQL** (GitHub) — If you're on GitHub, enable this immediately. It runs on every PR for free on public repos — I walk through the full setup, custom queries, and the private-repo pricing catch in my [GitHub code scanning with CodeQL guide](/blog/github-code-scanning-codeql-setup-free-alternatives).
