...
The compiler optimizes *re-renders*, not *initial mounts*. If clicking a button mounts a 500-component tree, the compiler won't help — that's an architectural issue. It's also worth remembering that the compiler is React's deliberate alternative to the [signals-based fine-grained reactivity](/blog/signals-fine-grained-reactivity-2026-guide) that every other major framework adopted — same goal of minimal wasted work, very different bet on how to get there.
...
**The rule of thumb:** For new code, don't add manual memoization. Let the compiler handle it. For existing code, don't rush to remove it — test first. When you're ready to clean out the old memoization, I wrote a [step-by-step removal playbook](/blog/react-compiler-remove-usememo-cleanup-playbook) that centers on the one dangerous case — deleting a `useMemo` an effect depends on.
