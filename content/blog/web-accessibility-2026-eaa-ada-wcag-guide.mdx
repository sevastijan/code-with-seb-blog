...
featured: false
author: 'Sebastian'
excerpt: 'With EAA now enforced, ADA deadlines in April 2026, and WCAG 3.0 on the horizon, accessibility is no longer optional. Here''s what you actually need to implement, with code examples and testing workflows that work.'
---
...
Everything on your site must be usable without a mouse. Worth knowing that visible keyboard focus is now one of three non-negotiables in [Anthropic's frontend-design skill](/blog/claude-code-frontend-design-plugin-guide), alongside mobile responsiveness and respecting reduced motion — a floor, not compliance, but it removes the most common regression at the point where the markup is written. This means:
